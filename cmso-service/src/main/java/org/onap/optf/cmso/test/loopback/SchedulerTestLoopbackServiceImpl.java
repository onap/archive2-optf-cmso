...
public class SchedulerTestLoopbackServiceImpl implements SchedulerTestLoopbackService {
    ...
    public Response putToOptimizer(OptimizerRequest request, UriInfo uri) {
        //
        try {
            OptimizerResponse r = new OptimizerResponse();
            r.setRequestId(request.getRequestId());
            r.setStatus(OptimizerResponse.OptimizeScheduleStatus.COMPLETED);
            List<OptimizerScheduleInfo> schedules = getSchedules(request);
            r.setSchedules(schedules);
            ObjectMapper om = new ObjectMapper();
            String response = om.writeValueAsString(r);
            optimizerResponses.put(r.getRequestId(),response);
    ...
    private List<OptimizerScheduleInfo> getSchedules(OptimizerRequest request) {
      ...
      // gave
      DateTime finishTime = new DateTime(cw.getEndTime().getTime());
      ...
      finishTime = latestInstanceStartTime.plus(totalDuration);
      ...
      try
      {
        CMSOOptimizerCallbackImpl.makeMap(startTime.getMillis(), latestInstanceStartTime.getMillis(), concurrencyLimit,
                totalDuration, nodeList, startAndFinishTimeMap);
    ...
    public Response getOptimizerResponse(String id, UriInfo uri) {
      //
      try {
          String r = optimizerResponses.get(id);
