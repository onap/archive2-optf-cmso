...
@Component
public class CMSOptimizerHandler {
      ...
      switch (status) {
        ...
        default:
          throw new CMSException(
              ...
              LogMessages.OPTIMIZER_CALLBACK_STATE_ERROR,
  ...
  private void saveSchedules(OptimizerResponse response, Schedule schedule) throws CMSException {
    ...
    for (ScheduledElement element : osi.getScheduledElements())
    {
      ...
      ChangeManagementSchedule cms = cmScheduleDAO.findOneByScheduleUUIDGroupIdAndVnfName(
              schedule.getUuid(), groupId, vnfName);
  ...
  private void updateGroup(
    ...
    try
    {
      ...
      if (cmg.getLastInstanceStartTimeMillis() == null || cmg.getLastInstanceStartTimeMillis() < elementStartTime)
      {
  ...
  private OptimizerScheduleInfo chooseSchedule(List<OptimizerScheduleInfo> schedules)
  {
    ...
    {
      if (chosenOne == null ||
          osi.getScheduledElements().size() > chosenOne.getScheduledElements().size())
      {
        chosenOne = osi;
      }
      else
