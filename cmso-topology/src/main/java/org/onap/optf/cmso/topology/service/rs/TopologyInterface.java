...
public interface TopologyInterface {
    ...
    @Path("/current/")
    @Produces({MediaType.APPLICATION_JSON})
    @RequestMapping(value = "/{apiVersion}/current", method = RequestMethod.POST, 
    		consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Request Topology", notes = "API to retrieve toplogy for scheduling 'conflict free' mainentance."
    		+ " Retrieves the element information related to the list of elements targeted for mainenance activity."
    		+ " Scope of related elements to be returned are defined in the passed ToplogogyRequest."
    		+ " Elements returned must include in the elementData, the identifier that the element is known as"
    		+ " in the ticket management system."
    		+ "\nThe Topology Service may implement asynchronous requests by returning IN_PROGRESS status."
    		+ " If IN_PROGRESS, the optimizer will begin polling until COMPLETED is returned with the response. ", 
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @RequestMapping(value = "/current/request/{id}", method = RequestMethod.GET, 
    		consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Poll Asynchronous Topology Request", notes = "If a topology request results in asynchronous request (IN_PROGRESS)"
    		+ " this GET is used to retrieve status until COMPLETED. At which time, the optimizer will "
    		+ " issue a DELETE to acknowledge receipt."
    		+ "\nThe Topology Service implementation may delete the cache when returning completed."
